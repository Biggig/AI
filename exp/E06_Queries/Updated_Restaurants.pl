...
area_has_meal(X, Y):-branch(A, X), restaurant(A, _, Y).
dis_has_meal(X,Y):-district(A, X), area_has_meal(A, Y).
dis_has_res(X,Y):-district(A,X),branch(Y,A).

num_res_tol(A,N):-bagof(B, branch(A,B),C),len(C,N). 
num_res_area(A,N):-bagof(B,branch(B,A),C),len(C,N).
num_res_dis(A,N):-bagof(B,dis_has_res(A,B),C),len(C,N).

more(A,B):-num_res_tol(A,N1),num_res_tol(B,N2),N1>N2,A\=B.
least(X):-not(more(X,_)).

longer(X,Y):-restaurant(X,Y1,_),restaurant(Y,Y2,_),Y1<Y2,X\=Y.
longest(X):-not(longer(_,X)).

sameDistrict(Res1, Res2):-dis_has_res(X,Res1),dis_has_res(X,Res2).
